...
async function fetchComponent(paths: string[], registryUrl: string) {
  try {
    const results = await Promise.all(
      paths.map(async (path) => {
        const url = getComponentUrl(path, registryUrl)
...
async function resolveRegistryDependencies(
  ...
  async function resolveDependencies(itemUrl: string) {
    ...
    try {
      const [result] = await fetchRegistry([url], config.registry)
...
export async function registryGetTheme(name: string, config: Config) {
  const baseColor = await getRegistryBaseColor(name, config.registry)
  if (!baseColor) {
    return null
  }
...
function getRegistryUrl(path: string, registryUrl: string) {
  ...
  return `${registryUrl}/public/r/${path}`
}

export function getComponentUrl(path: string, registryUrl: string) {
  if (isUrl(path)) {
    return path
  }

  return `${registryUrl}/registry/${path}`
}
